...
local function checkTable(key, val, player, xPlayers)
	...
	--	if (key == 'job' and player.job.name == value) or (key == 'job2' and player.job2.name == value) or player[key] == value then
...
function ESX.RefreshJob2s()
	local Job2s = {}
	local job2s = MySQL.query.await('SELECT * FROM job2s')

	for _, v in ipairs(job2s) do
		Job2s[v.name] = v
		Job2s[v.name].grades = {}
	end
	...
	for _, v in ipairs(job2Grades) do
		if Job2s[v.job2_name] then
			Job2s[v.job2_name].grades[tostring(v.grade2)] = v
		else
			print(('[^3WARNING^7] Ignoring job2 grades for ^5"%s"^0 due to missing job2'):format(v.job2_name))
		end
	end

	for _, v in pairs(Job2s) do
		if ESX.Table.SizeOf(v.grades) == 0 then
			Job2s[v.name] = nil
	...
	if not Job2s then
		-- Fallback data, if no jobs exist
		ESX.Job2s['unemployed2'] = { label = 'Unemployed2', grades = { ['0'] = { grade = 0, label = 'Unemployed', salary = 200, skin_male = {}, skin_female = {} } } }
	else
		ESX.Job2s = Job2s
	end
end
